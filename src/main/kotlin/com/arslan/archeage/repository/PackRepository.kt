...
interface PackRepository : JpaRepository<Pack,Long>{
    ...
    fun findByIdIn(ids: Collection<Long>) : List<Pack>

    @Query("""
        SELECT p1.id FROM Pack p1 JOIN p1.materials m1 WHERE :item IN (SELECT m2.item FROM Pack p2 JOIN p2.materials m2 WHERE p2.id = p1.id)
    ...
    """)
    fun selectPackIDsWithCraftingMaterial(item: Item, user: User) : List<Long>
