...
interface UserPriceRepository : JpaRepository<UserPrice,UserPriceKey> {
    ...
    @Query("""
        SELECT distinct p FROM UserPrice p WHERE p.id.purchasableItem in :items 
        AND p.timestamp = (SELECT max(p2.timestamp) FROM UserPrice p2 WHERE p2.id.purchasableItem = p.id.purchasableItem and p.id.user.id = :userID)
    """)
