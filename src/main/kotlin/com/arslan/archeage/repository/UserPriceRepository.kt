...
interface UserPriceRepository : JpaRepository<UserPrice,Long> {
    ...
    @Query("SELECT distinct p FROM UserPrice p JOIN FETCH p.purchasableItem WHERE p.purchasableItem in :items AND p.timestamp = (SELECT max(p2.timestamp) FROM UserPrice p2 WHERE p2.purchasableItem = p.purchasableItem AND p2.purchasableItem.archeageServer=:archeageServer) AND p.purchasableItem.archeageServer = :archeageServer")
    fun latestPrices(items: Collection<Item>,archeageServer: ArcheageServer) : List<UserPrice>

    @Query("SELECT distinct p FROM UserPrice p JOIN FETCH p.purchasableItem WHERE p.purchasableItem in :items AND p.timestamp = (SELECT max(p2.timestamp) FROM UserPrice p2 WHERE p2.purchasableItem = p.purchasableItem and p.user.id = :userID AND p2.purchasableItem.archeageServer=:archeageServer) AND p.purchasableItem.archeageServer = :archeageServer")
